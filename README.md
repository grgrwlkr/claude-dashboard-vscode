...
| `claudeStatusline.outputStyle` | `""` | Ask for an output style for the session — `default`, `Proactive`, `Explanatory`, `Learning`, `Concise`. There is no flag for it: `outputStyle` is a setting, so it travels as `--settings '{"outputStyle":"…"}'`, which merges with your settings files rather than replacing them. A style of your own goes through `launchArgs` by name |
