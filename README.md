...
| `claudeStatusline.launchSaveTo` | `""` | Also write the Launch tab's choices into a Claude Code settings file as the client's own keys: `user` is `~/.claude/settings.json`, `local` is the workspace's `.claude/settings.local.json`. Empty writes nothing; `max` effort and the extra arguments are not written. User settings only |
...
| `claudeStatusline.renameTabs` | `true` | The tab shows the session's name, from the title Claude Code writes on the terminal: it follows `/rename`, the generated name and the session opened in the agent view, in every tab. Needs `terminal.integrated.tabs.allowAgentCliTitle`, on by default. To keep the name after a window reload, set `"terminal.integrated.tabs.title": "${sequence}"`; without it a reloaded tab shows the client's version. Off, a terminal opened by **Open Claude Code** keeps one fixed name |
