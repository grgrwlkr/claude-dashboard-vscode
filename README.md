...
| `claudeStatusline.renameTabs` | `true` | The tab shows the session's name, from the title Claude Code writes on the terminal: it follows `/rename`, the generated name and the session opened in the agent view, in every tab. Needs `terminal.integrated.tabs.allowAgentCliTitle`, on by default. Off, a terminal opened by **Open Claude Code** keeps one fixed name |
